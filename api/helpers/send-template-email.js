module.exports = {
    ...
    fn: async function({ template, templateData, to, toName, subject, from, fromName, layout, ensureAck, bcc, attachments }) {
        ...
        if (dontActuallySend) {
            ...
            var subjectLinePrefix = sails.config.environment === 'production' ? '' : sails.config.environment === 'staging' ? '' : '';
            var messageData = {
            ...
            axios.post("http://localhost:85/Yetistiricimden/", messageData).then(res => {
